!!!    This program solves Double Mach reflection problem of 2D Euler equations by TENO scheme
!**********************************************************************
! Written by Sainath Ch, Email: 
!**********************************************************************
...
! Generate simple grid
 ...
 !***********************************************************************
	...
	double precision 	:: interp_poly5(0:2), alpha(0:2), beta(0:2), omega(0:2),v(-2:3,n_eqn),charstencil(-2:2)

	double precision	:: vd1(-2:3,n_eqn),char_ddx(-2:2),vd2(-2:3,n_eqn),char_ddx2(-2:2)

    double precision 	:: lefteigen(4,4), righteigen(4,4)
    double precision, parameter	:: epsilon = 1.0d-40, gamma = 1.4d0, Constant =1.0d00

    double precision	:: a(-3:NS+3), b(-3:NS+3), c1(-3:NS+3), r(-3:NS+3),temp(-3:NS+3)


	double precision    :: ul(n_eqn),ur(n_eqn),ul2(n_eqn),ur2(n_eqn)

  	double precision	:: tbv_weno(-ghostp:NS+ghostp,n_eqn),tbv_compact(-ghostp:NS+ghostp,n_eqn)

  	double precision	:: tbv_muscl(-ghostp:NS+ghostp,n_eqn)
...
! !WENO-Z by Borges 
            ! alpha(0) = 3.0d0/10.0d0 *  (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(0)))**power)
            ! alpha(1) = 6.0d0/10.0d0 * (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(1)))**power)
            ! alpha(2) = 1.0d0/10.0d0 *  (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(2)))**power)

           	! alpha(0) = 3.0d0/10.0d0  * (Constant + ((abs(beta(0)-beta(2)) / (epsilon + beta(0)))**power) + dx**(1.0d0/3.0d0)*((beta(0)+epsilon)/(tau+epsilon)) )
            ! alpha(1) = 6.0d0/10.0d0  * (Constant + ((abs(beta(0)-beta(2)) / (epsilon + beta(1)))**power) + dx**(1.0d0/3.0d0)*((beta(1)+epsilon)/(tau+epsilon)) )
            ! alpha(2) = 1.0d0/10.0d0  * (Constant + ((abs(beta(0)-beta(2)) / (epsilon + beta(2)))**power) + dx**(1.0d0/3.0d0)*((beta(2)+epsilon)/(tau+epsilon)) )
...
! !WENO-Z by Borges
            alpha(0) = (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(0))))**power
            alpha(1) = (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(1))))**power
            alpha(2) = (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(2))))**power

            teno(0)  =alpha(0)/(alpha(0)+alpha(1)+alpha(2))
            teno(1)  =alpha(1)/(alpha(0)+alpha(1)+alpha(2))
            teno(2)  =alpha(2)/(alpha(0)+alpha(1)+alpha(2))
            ...
            enddo
           	...
           	alpha(0) = (3.0d0/10.0d0 )*teno(0)
            alpha(1) = (6.0d0/10.0d0)*teno(1)
            alpha(2) = (1.0d0/10.0d0 )*teno(2)
            ...
            ul(i)=omega(0)*interp_poly5(0)+omega(1)*interp_poly5(1)+omega(2)*interp_poly5(2)
		...
		do i=1,n_eqn
			...
			interp_poly5(0)=charstencil(0)*(11.0/6.0d0)+charstencil(1)*(-7.0/6.0d0)+charstencil(2)*(2.0/6.0d0)
            interp_poly5(1)=charstencil(-1)*(2.0/6.0d0)+charstencil(0)*(5.0/6.0)+charstencil(1)*(-1.0/6.0d0)
            interp_poly5(2)=charstencil(-2)*(-1.0/6.0d0)+charstencil(-1)*(5.0/6.0)+charstencil(0)*(2.0/6.0d0)
...
!WENO-Z by Borges
            ! alpha(0) = 1.0d0/10.0d0 *  (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(0)))**power)
            ! alpha(1) = 6.0d0/10.0d0 * (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(1)))**power)
            ! alpha(2) = 3.0d0/10.0d0 *  (Constant + (abs(beta(0)-beta(2)) / (epsilon + beta(2)))**power)

           	! alpha(0) = 1.0d0/10.0d0  * (Constant + ((abs(beta(0)-beta(2)) / (epsilon + beta(0)))**power) + dx**(1.0d0/3.0d0)*((beta(0)+epsilon)/(tau+epsilon)) )
            ! alpha(1) = 6.0d0/10.0d0  * (Constant + ((abs(beta(0)-beta(2)) / (epsilon + beta(1)))**power) + dx**(1.0d0/3.0d0)*((beta(1)+epsilon)/(tau+epsilon)) )
            ! alpha(2) = 3.0d0/10.0d0  * (Constant + ((abs(beta(0)-beta(2)) / (epsilon + beta(2)))**power) + dx**(1.0d0/3.0d0)*((beta(2)+epsilon)/(tau+epsilon)) )
...
!WENO-Z by Borges
            ...
            enddo
           	...
           	alpha(0) = (1.0d0/10.0d0 )*teno(0)
            alpha(1) = (6.0d0/10.0d0)*teno(1)
            alpha(2) = (3.0d0/10.0d0 )*teno(2)

            omega(0)=alpha(0)/(alpha(0)+alpha(1)+alpha(2))
            omega(1)=alpha(1)/(alpha(0)+alpha(1)+alpha(2))
            omega(2)=alpha(2)/(alpha(0)+alpha(1)+alpha(2))

            ur(i)=omega(0)*interp_poly5(0)+omega(1)*interp_poly5(1)+omega(2)*interp_poly5(2)
